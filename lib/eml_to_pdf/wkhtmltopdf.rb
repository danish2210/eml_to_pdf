  ...
  class Wkhtmltopdf
    ...
    def self.convert(input, output_path)
      Timeout.timeout(EmlToPdf.configuration.timeout, ConversionTimeoutError) do
        IO.popen("#{EmlToPdf.configuration.wkhtmltopdf} --encoding utf-8 --footer-center [page] --footer-spacing 2.5 --load-error-handling ignore --load-media-error-handling ignore --quiet - #{output_path} 2>&1", "r+") do |pipe|
          pipe.puts(input)
          pipe.close_write
